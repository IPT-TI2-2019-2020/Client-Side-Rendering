...
export default class SubmitDialogComponent extends React.Component {
  ...
  handleSubmit(evt) {
    evt.preventDefault();
    if (this.toEdit) {
      bookService.update(this.props.book._id, this.state).then(() => this.props.submited(this.state));
    } else {
      bookService.create(this.state).then((bookId) => this.props.submited({ ...this.state, _id: bookId }));
    }
  }
  ...
  handleSubmit(evt) {
    ...
    if (this.toEdit) {
      ...
      bookService.update(_id, jsonData).then(() => {
        this.handleCoverSubmit({ ...jsonData, _id, cover });
      });
    } else {
      bookService.create(jsonData).then((result) => {
        this.handleCoverSubmit({ ...jsonData, _id: result._id });
      });
    }
  }
